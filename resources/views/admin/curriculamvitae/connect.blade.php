                        ...
                        <tbody>
                            @foreach($results as $result)
                                <tr>
                                    ...
                                    <td>
                                        @if(Auth::getUser()->role_id == 1)
                                            <a href="{{ url('detail/'.base64_encode($result->id)) }}" target="_blank" class="btn btn-info" >View CV</a>
                                            @if($result->connectstatus == 1)
                                                <button class="btn btn-primary">Accepted</button>
                                                <a href="{{ url('admin/curriculamvitae/'.$result->id.'/complete') }}" onClick = "return confirm('Are you sure want to complete this request');">
                                                    <button class="btn btn-success">Complete</button>
                                                </a>
                                            @else
                                                <a href="{{ url('admin/curriculamvitae/'.$result->id.'/accept') }}" onClick = "return confirm('Are you sure want to accept this request');">
